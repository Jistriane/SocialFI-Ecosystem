...
export function ContractEventsProvider({ children }: ContractEventsProviderProps) {
  const { t } = useLocale();
  ...
  useEffect(() => {
    ...
    // TrustChain Events
    const unsubReputationUpdate = on(SocketEvents.REPUTATION_UPDATE, (data: any) => {
    ...
    const unsubVerificationStatus = on(SocketEvents.VERIFICATION_STATUS, (data: any) => {
      updateVerificationStatus(data.address, data.isVerified);
    });

    // GovGame Events
    const unsubProposalCreated = on(SocketEvents.PROPOSAL_CREATED, (data: any) => {
      addProposal(data.proposal);
    });

    const unsubProposalUpdated = on(SocketEvents.PROPOSAL_UPDATED, (data: any) => {
    ...
    const unsubProposalExecuted = on(SocketEvents.PROPOSAL_EXECUTED, (data: any) => {
      updateProposalStatus(data.proposalId, 'executed');
    });

    const unsubProposalCancelled = on(SocketEvents.PROPOSAL_CANCELLED, (data: any) => {
    ...
    const unsubTradeCompleted = on(SocketEvents.TRADE_COMPLETED, (data: any) => {
      updateTrade(data.tradeId, { ...data.trade, status: 'completed' });
    });

    const unsubTradeCancelled = on(SocketEvents.TRADE_CANCELLED, (data: any) => {
    ...
    const unsubAchievementUnlocked = on(SocketEvents.ACHIEVEMENT_UNLOCKED, (data: any) => {
