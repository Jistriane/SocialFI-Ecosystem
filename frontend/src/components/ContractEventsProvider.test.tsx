...
describe('ContractEventsProvider', () => {
  ...
  it('deve registrar todos os event listeners quando conectado', () => {
    ...
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.REPUTATION_UPDATE, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.PROFILE_UPDATE, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.VERIFICATION_STATUS, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.PROPOSAL_CREATED, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.PROPOSAL_UPDATED, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.VOTE_CAST, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.PROPOSAL_EXECUTED, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.PROPOSAL_CANCELLED, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.TRADE_CREATED, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.TRADE_UPDATED, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.TRADE_COMPLETED, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.TRADE_CANCELLED, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.REWARD_EARNED, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.LEVEL_UP, expect.any(Function));
    expect(mockOn).toHaveBeenCalledWith(SocketEvents.ACHIEVEMENT_UNLOCKED, expect.any(Function));
  });
  ...
  it('deve atualizar perfil quando receber evento PROFILE_UPDATE', () => {
    ...
    expect(mockStoreActions.updateProfile).toHaveBeenCalledWith('0x123', { username: 'test' });
  });
  ...
  it('deve atualizar status de verificação quando receber evento VERIFICATION_STATUS', () => {
    ...
    expect(mockStoreActions.updateVerificationStatus).toHaveBeenCalledWith('0x123', true);
  });
  ...
  it('deve adicionar proposta quando receber evento PROPOSAL_CREATED', () => {
    ...
    expect(mockStoreActions.addProposal).toHaveBeenCalledWith({ id: 1, title: 'Test Proposal' });
  });
  ...
  it('deve atualizar proposta quando receber evento PROPOSAL_UPDATED', () => {
    ...
    expect(mockStoreActions.updateProposal).toHaveBeenCalledWith(1, { id: 1, status: 'updated' });
  });
  ...
  it('deve adicionar voto quando receber evento VOTE_CAST', () => {
    ...
    expect(mockStoreActions.addVote).toHaveBeenCalledWith(1, { proposalId: 1, voter: '0x123' });
  });
  ...
  it('deve atualizar status da proposta quando receber eventos de execução ou cancelamento', () => {
    ...
    expect(mockStoreActions.updateProposalStatus).toHaveBeenCalledWith(1, 'executed');
    expect(mockStoreActions.updateProposalStatus).toHaveBeenCalledWith(1, 'cancelled');
  });
  ...
  it('deve gerenciar trades corretamente', () => {
    ...
    expect(mockStoreActions.updateTrade).toHaveBeenCalledWith(1, { id: 1, status: 'completed' });
    expect(mockStoreActions.updateTrade).toHaveBeenCalledWith(1, { id: 1, status: 'cancelled' });
  });
  ...
  it('deve gerenciar recompensas e conquistas corretamente', () => {
    ...
    expect(mockStoreActions.addAchievement).toHaveBeenCalledWith({ achievement: 'test' });
  });
}); 