...
import { Shield, Users, Star, Activity, Award, TrendingUp, CheckCircle, AlertCircle, X, ExternalLink, Loader2 } from 'lucide-react'
  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      try {
        ...
        toast({
          title: "Erro",
          description: "Não foi possível carregar as estatísticas do TrustChain",
          variant: "destructive"
        })
      } finally {
        setLoading(false)
      ...
      try {
        ...
        if (profile && profile.exists) {
          setUserProfile({
            ...
            connectionsCount: Number(profile.connectionsCount) || 0,
            transactionsCount: Number(profile.transactionsCount) || 0,
            reputation: getReputationLevel(Number(score) || 0),
            joinDate: Number(profile.createdAt) || Date.now()
          })
        } else {
          setUserProfile({
            exists: false,
      ...
      <div className="bg-slate-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-700">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-white">Sobre o TrustChain</h2>
            <button
          ...
          <div className="space-y-6 text-slate-300">
            <section>
              <h3 className="text-xl font-semibold text-white mb-3">O que é o TrustChain?</h3>
              <p>
                O TrustChain é um sistema revolucionário de reputação descentralizado que utiliza a tecnologia blockchain 
                para criar uma rede de confiança transparente e verificável. Cada interação, conexão e transação é 
                registrada de forma imutável, construindo um perfil de confiança único para cada usuário.
              </p>
            </section>

            <section>
              <h3 className="text-xl font-semibold text-white mb-3">Como Funciona?</h3>
              <div className="space-y-3">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-white">Criação de Perfil</h4>
                    <p>Conecte sua carteira e crie seu perfil TrustChain único na blockchain.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-white">Estabeleça Conexões</h4>
                    <p>Conecte-se com outros usuários através de transações, colaborações ou validações mútuas.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-white">Construa Reputação</h4>
                    <p>Sua pontuação de confiança cresce automaticamente com cada interação bem-sucedida.</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <p>
                Construído na blockchain Metis Hyperion, o TrustChain utiliza contratos inteligentes avançados 
                para garantir a segurança e eficiência das operações. A integração com o ecossistema SocialFI 
                permite uma experiência fluida entre diferentes aplicações descentralizadas.
              </p>
            </section>
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="text-center space-y-6 py-12">
            ...
            <p className="text-xl text-slate-300 max-w-3xl mx-auto leading-relaxed">
              Sistema de reputação descentralizado que constrói confiança através de conexões 
              verificadas e validadas na blockchain.
            </p>
          </div>
          ...
          {!isConnected && (
            <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6 text-center">
              ...
              <p className="text-slate-300 mb-4">Para acessar o TrustChain, você precisa conectar sua carteira primeiro.</p>
            ...
            <>
              ...
              <div className="grid md:grid-cols-4 gap-6">
                <div className="glass-card p-6 text-center group hover:scale-105 transition-all duration-300">
                  ...
                  <div className="text-3xl font-bold text-metis-400">
                    {loading ? <Loader2 className="w-6 h-6 animate-spin mx-auto" /> : (stats?.totalUsers || 0)}
                  </div>
                  <div className="text-slate-400 text-sm">Usuários Verificados</div>
                </div>
                ...
                <div className="glass-card p-6 text-center group hover:scale-105 transition-all duration-300">
                  ...
                  <div className="text-3xl font-bold text-hyperion-400">
                    {loading ? <Loader2 className="w-6 h-6 animate-spin mx-auto" /> : (stats?.totalConnections || 0)}
                  </div>
                  <div className="text-slate-400 text-sm">Conexões de Confiança</div>
                </div>
                ...
                <div className="glass-card p-6 text-center group hover:scale-105 transition-all duration-300">
                  ...
                  <div className="text-3xl font-bold text-neon-green">
                    {loading ? <Loader2 className="w-6 h-6 animate-spin mx-auto" /> : `${stats?.trustRate || 0}%`}
                  </div>
                  <div className="text-slate-400 text-sm">Taxa de Confiança</div>
                </div>
                ...
                <div className="glass-card p-6 text-center group hover:scale-105 transition-all duration-300">
                  ...
                  <div className="text-3xl font-bold text-neon-yellow">
                    {loading ? <Loader2 className="w-6 h-6 animate-spin mx-auto" /> : (stats?.totalTransactions || 0)}
                  </div>
                  <div className="text-slate-400 text-sm">Transações Validadas</div>
                </div>
              </div>
              ...
              {userProfile && !userProfile.exists && (
                <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-white mb-2">Crie seu Perfil TrustChain</h3>
                  <p className="text-slate-300 mb-4">Você ainda não possui um perfil no TrustChain. Crie agora para começar a construir sua reputação!</p>
                  <Button
                    onClick={() => {
              ...
              <div className="glass-card p-8">
                {activeTab === 'overview' && userProfile && (
                  <div className="space-y-6">
                    <h2 className="text-2xl font-bold text-white mb-6">Visão Geral do Perfil</h2>
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">Nome de Usuário:</span>
                          <span className="text-white font-medium">{userProfile.username || 'Não definido'}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">Pontuação de Confiança:</span>
                          <span className="text-metis-400 font-bold">{userProfile.trustScore}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">Nível de Reputação:</span>
                          <span className="text-neon-green font-medium">{userProfile.reputation}</span>
                        </div>
                      </div>
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">Conexões:</span>
                          <span className="text-white font-medium">{userProfile.connectionsCount}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">Transações:</span>
                          <span className="text-white font-medium">{userProfile.transactionsCount}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">Membro desde:</span>
                          <span className="text-white font-medium">{new Date(userProfile.joinDate).toLocaleDateString()}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeTab === 'connections' && (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-xl font-semibold text-white mb-2">Conexões em Desenvolvimento</h3>
                ...
                {activeTab === 'reputation' && userProfile && (
                  <div className="space-y-6">
                    <h2 className="text-2xl font-bold text-white mb-6">Sistema de Reputação</h2>
                    <div className="bg-slate-700/50 rounded-lg p-6">
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-slate-300">Pontuação Atual</span>
                        <span className="text-2xl font-bold text-metis-400">{userProfile.trustScore}</span>
                      </div>
                      <div className="w-full bg-slate-600 rounded-full h-3">
                        <div 
                          className="bg-gradient-to-r from-metis-500 to-neon-green h-3 rounded-full transition-all duration-500"
                          style={{ width: `${Math.min((userProfile.trustScore / 1000) * 100, 100)}%` }}
                        ></div>
                      </div>
                ...
                {activeTab === 'activity' && (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-xl font-semibold text-white mb-2">Histórico de Atividades</h3>
                    <p className="text-slate-400">Esta funcionalidade será implementada em breve.</p>
                  </div>
                )}
              </div>
