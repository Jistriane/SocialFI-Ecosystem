...
describe('useTranslation', () => {
  ...
  it('should return translation with variables', () => {
    const { result } = renderHook(() => useTranslation('trustchain'), {
      wrapper,
    })
  ...
  it('should switch languages correctly', () => {
    ...
    const { result: translationResult } = renderHook(
  ...
  it('should handle nested translations', () => {
    ...
    expect(result.current.t('dashboard.active_proposals')).toBe(
      'Propostas Ativas',
    )
    expect(result.current.t('proposals.voting_period')).toBe(
      'Período de Votação',
    )
  })
})
